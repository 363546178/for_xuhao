...
void EKF::predict(const Quaterniond q_eb, const Vector3d w_b, const Vector3d a_e, const float dt)
{
    if(is_init == true)
    {
        ...
        F_mat<<
                ...
                R_vb*crossMat(get_bias_a()), Matrix3d::Zero(),     Matrix3d::Zero(),       -R_ve*R_eb,
...
void EKF::correct()
{
    ...
    x.segment<3>(0+4+3) += delta_v_v;
    x.segment<3>(0+4+3+3) += delta_bias_v;
